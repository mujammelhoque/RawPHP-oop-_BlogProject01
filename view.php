...
$userid = Session::getSessionData("userid");
 ...
 $inQ= "INSERT INTO `comments` (`id`, `tid`, `uid`,`mid`,`pid`, `comment`, `status`, `created`, `deleted`) VALUES (NULL, NULL, '.$userid.',NULL,'.$id.', '.$c.','3', current_timestamp(), NULL)";
...
if(isset($_GET['id'])){
  ...
  <?php 
     ...
     $qw = "SELECT poetpoem.*,poetcata.name  FROM `poetpoem` 
     inner join poetcata on poetpoem.pcid=poetcata.id
     WHERE  poetpoem.id=".$id." ";
     $share = $conn->query($qw);
...
<meta property="og:image"         content="<?php echo Config::siteinfo()['baseurl'] ?>poetimage/<?php echo $info['image'];?>" />
...
<h3>All Comment</h3>
<?php 
$ssl= "SELECT comments.*,users.username FROM comments inner join users on comments.uid=users.id where comments.pid=$id ";
$qq = $conn->query($ssl);
if ($qq->num_rows >0) {
