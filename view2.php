...
$userid = Session::getSessionData("userid");
 ...
 $inQ= "INSERT INTO `comments` (`id`, `tid`, `uid`,`mid`,`pid`, `comment`, `created`, `deleted`) VALUES (NULL, '.$id.', '.$userid.',NULL,NULL, '.$c.', current_timestamp(), NULL)";
...
<meta property="og:image"         content="<?php echo Config::siteinfo()['baseurl'] ?>pubimage/<?php echo $info['image'];?>" />
...
<div>';
if($userid==$row['uid']){
echo '<a class="btn rounded-pill btn-sky mb-3" href="update.php?tid='.$id.'"><h4 class="btn btn-outline-primary text-success">Update </h4></a>
<a class="btn rounded-pill btn-sky mb-3" href="delete.php?tid='.$id.'"><h4 class="btn btn-outline-primary text-success">Delete </h4></a>';}
// else{
//   echo '<a class="btn rounded-pill btn-sky mb-3" href="#"><h4 class="btn btn-outline-primary text-success">Update </h4></a>
// <a class="btn rounded-pill btn-sky mb-3" href="#"><h4 class="btn btn-outline-primary text-success">Delete </h4></a>';
// }
echo'
</div>
<div class="" data-href="<?php '.$url.'; ?>" data-layout="button_count" data-size="large"><h4 ><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u='.$url.';" class="fb-xfbml-parse-ignore"><img style="width:50px" src="image/download.png" alt=""></a></h4></div>
</div>'
;}
...
<h3>All Comment</h3>
<?php 
$ssl= "SELECT comments.*,users.username FROM comments inner join users on comments.uid=users.id where comments.tid=$id";
$qq = $conn->query($ssl);
if ($qq->num_rows >0) {
