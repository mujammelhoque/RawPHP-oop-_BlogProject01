...
<div class="collapse navbar-collapse" id="navbarTogglerDemo01" class="">
    ...
    $increment = $defaultpage +1;
       ...
       $pagi = "SELECT COUNT(*) AS total FROM ptweets WHERE uid=".$userid." AND cid={$cid} AND privacy='1' ";
       ...
       $q = "SELECT ptweets.*,mypoemcata.name  FROM `ptweets` 
                 ...
                 WHERE  ptweets.cid=".$cid ." AND ptweets.uid=$userid and ptweets.privacy='1' order by ptweets.created desc LIMIT " . $recordStart . "," . PAGE_SIZE . "";
     
       
       $allTweet = $conn->query($q);

     if ($allTweet->num_rows>0) {
     echo '<h3>Published by '.Session::getSessionData("username").'</h3>';
  ...
  <div class="row g-0">
    ...
    <div class="col-md-6">
      <div class="card-body">
        ...
        <p class="card-text"><pre><pre>'. mb_substr($row['details'], 0, 200) .'......</pre></pre></p>
  ...
  <ul class="pagination">
            ...
            if ($defaultpage <= 1) {
              ...
              echo '<li class="page-item "><a class="page-link" href="'.$_SERVER['PHP_SELF'] .'?page='.$decrement.'&cid='.$cid.' ">Previous</a></li>';
                  ...
                  echo '<li class="page-item"><a class="page-link active " href="'.$_SERVER['PHP_SELF'] .'?page='.$i.'&cid='.$cid.' ">' . $i . '</a></li>';
                 
								} else {
									// echo '<a class="page" href="' . $_SERVER['PHP_SELF'] . '?page=' . $i . '&cid='.$mpcid.'">' . $i . '</a>';
                  echo '<li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'] .'?page='.$i.'&cid='.$cid.' ">' . $i . '</a></li>';
								}
						}
						}

            if ($defaultpage ==$totalPage) {
              echo '<li class="page-item disabled"><a class="page-link" href="'.$_SERVER['PHP_SELF'] .'?page='.$increment.'&cid='.$cid.' ">Next</a></li>';

            }else{
              echo '<li class="page-item "><a class="page-link" href="'.$_SERVER['PHP_SELF'] .'?page='.$increment.'&cid='.$cid.' ">Next</a></li>';
