                ...
                <div class="row my-4">
                    ...
                    <div class="col-12 col-md-6 mb-4 mb-lg-0 col-lg-3">
                        <div class="card">
                            ...
                            <?php
                            $q = "SELECT count(*) as totaltweets FROM ptweets";
                            $queryResult = $conn->db->query($q);
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                        <?php
                                         $q = "SELECT ptweets.*,users.username , users.email, mypoemcata.name FROM `ptweets` LEFT OUTER join users on ptweets.uid=users.id JOIN mypoemcata on ptweets.cid=mypoemcata.id WHERE ptweets.privacy='1' ORDER BY ptweets.created DESC LIMIT 10";
                                        ...
                                        if ($tweetsQuery->num_rows>0) {
                                          while ($row= $tweetsQuery->fetch_assoc()) {
                                            ...
                                            <td>'.$row['username'].'</td>
                                            <td>'.$row['title'].'</td>
                                            <td style="width:80px"><img style="width:50px" src="../pubimage/'.$row['image'].'" alt="'.$row['title'].'"></td>
                                            <td>'.$row['email'].'</td>
                                            <td>'.$row['name'].'</td>
                                            <td>'.$row['created'].'</td>
                                            <td><a href="adview.php?tid='.$row['id'].'" class="btn btn-sm btn-primary">View</a> <a href="edit.php?tid='.$row['id'].'" class="btn btn-sm btn-warning">Edit</a> <a href="delete.php?tid='.$row['id'].'" class="btn btn-sm btn-danger">Delete</a> </td>
                                          </tr>';
                                          }
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                        <?php
                                        $q = "SELECT * FROM users where status='1' order by created desc limit 10 ";
                                        $tweetsQuery = $conn->db->query($q);
                                        if ($tweetsQuery->num_rows>0) {
                                          while ($row= $tweetsQuery->fetch_assoc()) {
                                            // echo $row['username'];
                                            ...
                                            <td>'.$row['email'].'</td>
                                             <td>'.$row['created'].'</td>
                                            <td> <a href="uedit.php?uid='.$row['id'].'" class="btn btn-sm btn-warning">Edit</a> <a href="?id='.$row['id'].'" class="btn btn-sm btn-danger">Delete</a>
