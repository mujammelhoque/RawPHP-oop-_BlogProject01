...
$sqltotal = "SELECT COUNT(*) AS total FROM ptweets WHERE uid=$userid AND privacy='1'
";
...
$sql = "SELECT ptweets.*, mypoemcata.name ,count(*) as total FROM `ptweets` 
							inner join mypoemcata on ptweets.cid=mypoemcata.id WHERE  ptweets.uid=".$userid ." AND ptweets.privacy='1'  group by ptweets.`cid`";
							$allCategory = $conn->query($sql);
							echo ' <ul class="list-group">';
							echo '<li class="list-group-item d-flex justify-content-between align-items-center"> <h4>
						My Posts 	&nbsp <h4> <span class="badge bg-primary rounded-pill">'.$total.'</span></li>';
						echo ' <li class="list-group-item d-flex justify-content-between align-items-center"> <h4><a class="text-decoration-none" href="insertweet.php">Add New</a><h4></li>';
